...
async function viewProfile(userId) {
  if (!userId || userId === "undefined" || userId === "null") {
    console.warn("⚠️ Invalid profile ID");
    return;
  }
  ...
  try {
    ...
    if (cover) {
      ...
      cover.src = user.coverImage ? buildImage(user.coverImage) : "";
    }
    ...
    if (bioText) bioText.innerText = user.bio || "No bio yet";

    const followers = document.getElementById("followersCount");
    if (followers) followers.innerText = user.followers?.length || 0;

    const following = document.getElementById("followingCount");
    if (following) following.innerText = user.following?.length || 0;
    ...
    if (profile) {
      profile.innerHTML = `
        ...
        <div class="bg-slate-50 rounded-2xl p-4 border border-slate-100 shadow-inner">
          ...
          <p class="text-slate-700 text-sm leading-relaxed">${user.about || "No description"}</p>
        </div>
      `;
    }
...
function renderPosts(posts) {
  ...
  container.innerHTML = posts.map(post => {
    ...
    const authorId = String(postAuthor._id || postAuthor.id || "");
    const authorTarget = authorId ? (authorId === currentUserId ? "profile.html" : `user.html?id=${authorId}`) : "#";
        ...
        <div class="p-5">
          ...
          <div class="flex items-center gap-3 mb-4 cursor-pointer group" onclick="if('${authorTarget}' !== '#') window.location.href='${authorTarget}';">
            <div class="w-10 h-10 rounded-full overflow-hidden bg-slate-100 border border-slate-200 flex items-center justify-center">
              ${postAuthor.profilePic ? `
                <img src="${buildImage(postAuthor.profilePic)}" class="w-full h-full object-cover" onerror="this.onerror=null; this.src='images/default-avatar.png';" />
              ` : `
            ...
            <div>
              <p class="font-bold text-slate-800 text-sm group-hover:text-blue-600 transition">${postAuthor.username || "Unknown User"}</p>
          ...
          <div class="space-y-2 mb-4 max-h-48 overflow-y-auto pr-1">
            ${(post.comments || []).map(comment => {
              ...
              const commentTarget = commentAuthorId ? (commentAuthorId === currentUserId ? "profile.html" : `user.html?id=${commentAuthorId}`) : "#";
          ...
          <div class="flex gap-2 border border-slate-200/80 p-1 rounded-xl bg-slate-50/30">
            <input id="comment-${post._id}" placeholder="Write a comment..." class="flex-1 bg-transparent px-3 py-1.5 text-xs outline-none text-slate-700" />
            <button onclick="commentPost('${post._id}')" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-1.5 rounded-lg text-xs font-bold">Send</button>
          </div>
        </div>
      </div>
    `;
...
function setupFilePreviews() {
  ...
  if (coverImageInput) {
    coverImageInput.addEventListener("change", (e) => {
      ...
      if (file) {
        const cover = document.getElementById("coverPreview");
        if (cover) cover.src = URL.createObjectURL(file);
      }
    });
  }
}
